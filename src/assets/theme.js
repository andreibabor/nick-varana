    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant) {
        if (variant.available) {
          ...
          $('.prenote-description').remove();
          $('.line_item_properties').remove();
          // bbb code
          var prenoteID = '#preorder_'+variant.id;
          if ($(prenoteID).length) {
            $(prenoteID).append("<p class='prenote-description'>This item is out of stock, but you can order it and will receive it in 2-3 weeks.</p>");
            $('#AddToCartText-product-template').text('pre-order');
            $('.product-form-product-template').append("<input class='line_item_properties' type='hidden' name='properties[Note]' value='Pre-order item' />");
          }else{
           $('.prenote-description').remove();
           $('.line_item_properties').remove();
         }
